    ...
    <div className="space-y-8">
      <section className="rounded-4xl border border-slate-200 bg-white p-8 shadow-soft dark:border-slate-700 dark:bg-slate-900">
        <div className="max-w-3xl space-y-4">
          <p className="text-sm uppercase tracking-[0.4em] text-slate-500 dark:text-slate-400">Voting Dashboard</p>
          <h1 className="text-4xl font-semibold text-slate-900 dark:text-slate-100">Daily anonymous voting for Agric Class 29</h1>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link to="/results" className="rounded-2xl border border-slate-200 px-5 py-3 text-slate-700 transition hover:border-slate-400 dark:border-slate-700 dark:text-slate-300 dark:hover:border-slate-600">
              View results
            </Link>
          </div>
        </div>
      ...
      <section className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <h2 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Candidates</h2>
          <span className="rounded-full bg-slate-100 px-4 py-2 text-sm text-slate-700 dark:bg-slate-800 dark:text-slate-300">{candidates.length} candidates</span>
        </div>
        {loading ? (
          <LoadingSpinner />
        ) : candidates.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white/80 p-12 text-center text-slate-600 dark:border-slate-700 dark:bg-slate-900/50 dark:text-slate-400">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            {Object.entries(grouped).map(([position, items]) => (
              <div key={position} className="space-y-4">
                <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-200">{position}</h3>
